...
class TCN(nn.Module, TimeSeriesDataset):
    def __init__(
        self,
        L_in,
        L_out,
        ...
        **kwargs
        ...
        self.tcn = TemporalConvNet(d_model, num_channels, kernel_size, dropout)
    ...
    def forward(self, x):
        ...
        # x: [N, C_in, L_in]
        x = self.embedding(x_reals, x_cats).transpose(1, 2)
        # y1: [N, C_out, L_in] ==> [N, L_in, C_out]
        y1 = self.tcn(x).transpose(1, 2)
        # out: [N, L_out, C_out] ==> [N, L_out, output_size]
        out = self.linear(y1[:, -self.L_out :, :])
