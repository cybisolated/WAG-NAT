...
def objective(trial: optuna.trial.Trial):
    ...
    L_in_list = [24, 48, 168, 336]
    L_out = args.L_out
    L_in = trial.suggest_categorical('L_in', [d for d in L_in_list if d >= L_out]) if args.L_in is None else args.L_in
    ...
    n_levels = trial.suggest_categorical('n_levels', [1, 2, 3, 4, 5])
    ...
    model = PlModelInterface(model=tcn, **vars(args))
